...
const ShiftCard = () => {
  ...
  const shifts = [
    // { time: "12:00am - 10:30am EDT", SkillsPreferred: "LAMP Stack", status: "Late Night", rate: "$26.00/hour" },
   ...
   { time: "4:00pm - 2:30am EDT", SkillsPreferred: "LAMP Stack" , status: "Late Night"},
    ...
    // { time: "1:30am - 12:00pm EDT", SkillsPreferred: "Java" },
     ...
     { time: "2:00am - 1:30pm EDT", SkillsPreferred: "Java", status: "Late Night" },
    ...
    // { time: "9:30am - 8:00pm EDT", SkillsPreferred: "Java" },
     { time: "1:30am - 12:00pm EDT", SkillsPreferred: "Python", status: "Late Night" },
    // { time: "11:30am - 10:00pm EDT", SkillsPreferred: "Python" },
     { time: "2:00am - 1:30pm EDT", SkillsPreferred: "Python", status: "Late Night" },
  ...
  const filteredShifts = shifts.filter(shift => {
    // Check if the shift's SkillsPreferred matches any of the selected skill filters
    const skillMatch = Object.keys(filters).some(filter => 
      filters[filter] && shift.SkillsPreferred.includes(filter)
    ) || !Object.keys(filters).some(filter => filters[filter]);
  
    // Assuming 'status' is a simple string, check if the shift's status matches any of the selected status filters
    const statusMatch = filters[shift.status] || !filters["Late Night"];
